...
public class TypeConverterTest {
    ...
    public void testBasicTypeConversion() {
        ...
        // 测试字符串转整数
        assertEquals(42, converter.convertIfNecessary("42", Integer.class));

        // 测试字符串转长整数
        assertEquals(123L, converter.convertIfNecessary("123", Long.class));

        // 测试字符串转双精度浮点数
        assertEquals(3.14, converter.convertIfNecessary("3.14", Double.class));

        // 测试字符串转单精度浮点数
        assertEquals(3.14f, converter.convertIfNecessary("3.14", Float.class));

        // 测试字符串转布尔值
        assertTrue(converter.convertIfNecessary("true", Boolean.class));
        assertTrue(converter.convertIfNecessary("yes", Boolean.class));
        ...
        // 测试数字转字符串
        assertEquals("42", converter.convertIfNecessary(42, String.class));
    }
    ...
    public void testDateTimeConversion() {
        ...
        assertEquals(LocalDate.parse("2023-12-25"),
                converter.convertIfNecessary("2023-12-25", LocalDate.class));

        // 测试字符串转LocalTime
        assertEquals(LocalTime.parse("12:34:56"),
                converter.convertIfNecessary("12:34:56", LocalTime.class));

        // 测试字符串转LocalDateTime（ISO格式）
        assertEquals(LocalDateTime.parse("2023-12-25T12:34:56"),
                converter.convertIfNecessary("2023-12-25T12:34:56", LocalDateTime.class));

        // 测试字符串转LocalDateTime（自定义格式）
        assertEquals(LocalDateTime.parse("2023-12-25T12:34:56"),
                converter.convertIfNecessary("2023-12-25 12:34:56", LocalDateTime.class));
    }
    ...
    public void testInvalidConversion() {
        ...
        // 测试无效的日期格式
        assertThrows(TypeMismatchException.class, () -> {
            converter.convertIfNecessary("not a date", LocalDate.class);
        });
    }
            ...
            executorService.submit(() -> {
                try {
                    ...
                    for (int j = 0; j < iterationsPerThread; j++) {
                        assertEquals(j, localConverter.convertIfNecessary(String.valueOf(j), Integer.class));
                    }
                } finally {
                    latch.countDown();
                }

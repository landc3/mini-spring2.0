...
public class AbstractEnvironment implements ConfigurableEnvironment{
    ...
    private String[] getProfilesFromProperty(String propertyName) {
        String profileStr = getProperty(propertyName);// 获取属性值
        if (profileStr != null) {
            return profileStr.split(",");//  返回profiles数组
        }
        return null;
    }
    /**
    ...
    public void setActiveProfiles(String... profiles) {
        ...
        for (String profile : profiles) {
            validateProfile(profile); // 验证profile
        }
        this.activeProfiles.clear();
    ...
    public void addActiveProfile(String profile) {
        ...
        this.activeProfiles.add(profile);// 添加活跃的profile
    }
    /**
     * 设置默认的profiles
     *
    ...
    public String[] getDefaultProfiles() {
        if (this.defaultProfiles.isEmpty()) {// 如果默认的profiles为空，则返回一个空的数组
            return new String[0];
        }
        return this.defaultProfiles.toArray(new String[0]);//  返回默认的profiles数组
    }
    /**
     * 获取活跃的profiles
     */

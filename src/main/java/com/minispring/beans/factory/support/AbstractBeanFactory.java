...
public abstract class AbstractBeanFactory extends DefaultSingletonBeanRegistry implements BeanFactory {
    ...
    /**
     * 为什么要在 AbstractBeanFactory添加BeanPostProcessor的管理功能？
     * AbstractBeanFactory 是 Spring 容器的核心类之一，负责 Bean 的创建和管理。
     * 将 BeanPostProcessor 的管理功能集成到 AbstractBeanFactory 中，可以确保所有 Bean 的生命周期都受到统一的控制。
     * 通用性：
     * 所有子类（如 DefaultListableBeanFactory）都可以继承并使用这些功能，从而实现一致的行为。
     */
    /** BeanPostProcessor列表 */
    ...
    protected <T> T doGetBean(String name, Class<T> requiredType, Object[] args) throws BeansException {
        ...
        if (beanDefinition.isSingleton()) { // 判断是否为单例Bean
